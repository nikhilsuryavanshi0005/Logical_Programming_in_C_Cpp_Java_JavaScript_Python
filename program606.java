        ...
        if((fobj.exists()) && (fobj.isDirectory()))
        {
            ...
            for( i =0; i<fArr.length; i++)
            {
                ...
                if(fArr[i].getName().endsWith(".txt"))
                {
                    ...
                    //Write header into PackFile
                    foobj.write(bHeader,0,100);
                  

                    while((iRet = fiobj.read(Buffer)) != -1)   //Read the data from input files from Marvellous folder
                    {
                        for(j =0; j<iRet; j++) //Encrpytion logic
                        {
                            Buffer[j] = (byte)(Buffer[j] ^ Key);
                        }

                        //write the files data from the Folder
